...
```
module github.com/guilhermegouw/cdd  # Module path (import path)
...
- **Subcommand support:** Natural `cdd <command>` structure.
...
```bash
go build -ldflags "-X github.com/guilhermegouw/cdd/cmd.Version=1.0.0"
```
...
- **CDD builds:** Test on multiple OS/Go versions in parallel.
...
| `task clean` | `rm -f cdd` | Remove build artifacts |
